...
document.addEventListener("DOMContentLoaded", () => {
  ...
  accordionBodies.forEach((body) => {
    ...
    if (section) {
      fetch(`components/${section}.html`).then((response) => {
        if (!response.ok) {
        ...
        body.innerHTML = html;
        initializeComponentScripts(body);
      }).catch((error) => {
...
function initializeComponentScripts(container) {
  ...
  if (selects.length) {
    selects.forEach((select) => {
      select.innerHTML = `<option disabled selected value="">Select</option>` + commodities.map((item) => `<option value="${item}">${item}</option>`).join("");
    });
    selects.forEach((select) => {
